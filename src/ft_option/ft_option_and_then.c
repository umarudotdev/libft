...
/**
 * @brief Returns None if the option is None, otherwise calls f with the wrapped
 * value and returns the result.
 *
 * This function enables monadic chaining of operations that return Options.
 * It's useful for composing multiple operations that might fail, allowing
 * for clean error propagation without nested conditionals.
 *
 ...
 */
t_option	ft_option_and_then(t_option option, t_option (*f)(void *))
{
	if (!f)
