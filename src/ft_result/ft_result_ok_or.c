 ...
 *
 * This function transforms a Result into an Option by converting Ok(value)
 * to Some(value) and Err(_) to None. This is useful when you only care
 * about whether an operation succeeded, not the specific error.
 *
...
{
	if (result.is_ok)
		return (ft_option_some(result.ok_value));
