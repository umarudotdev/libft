 ...
 *
 * This function applies the given function to the Err value if the Result
 * is Err, allowing for error recovery. If the Result is Ok, it returns
 * the Ok unchanged. This is useful for fallback operations.
 *
 ...
 */
t_result	ft_result_or_else(t_result result, t_result (*f)(void *))
{
	if (!f)
		return (result);
	if (!result.is_ok)
		return (f(result.err_value));
	return (result);
}
