 ...
 * function to it. If the Result is Ok, it returns the Ok unchanged.
 * This is useful for error recovery and transformation.
 *
...
t_result	ft_result_map_err(t_result result, void *(*f)(void *))
{
	if (!f)
		return (result);
	if (!result.is_ok)
		return (ft_result_err(f(result.err_value)));
	return (result);
}
