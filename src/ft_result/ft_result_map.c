 ...
 * function to it. If the Result is Err, it returns the Err unchanged.
 * This enables functional programming patterns and method chaining.
 *
...
t_result	ft_result_map(t_result result, void *(*f)(void *))
{
	if (!f)
		return (result);
	if (result.is_ok)
		return (ft_result_ok(f(result.ok_value)));
	return (result);
}
