 ...
 *
 * This function implements the monadic bind operation for Results.
 * It applies the given function to the Ok value if the Result is Ok,
 * and returns the Err unchanged if the Result is Err. This allows
 ...
 */
t_result	ft_result_and_then(t_result result, t_result (*f)(void *))
{
	if (!f)
